...
export default function PlayScreen({ names, timeLeft, onVote }) {
  const danger = timeLeft <= 10;
  useEffect(() => {
    if (danger && timeLeft > 0) sounds.tick();
  }, [timeLeft, danger]);
  return <main className="flex min-h-screen items-center justify-center px-5 py-10"><motion.div initial={{ scale: .9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-lg rounded-[2.5rem] border border-white/70 bg-white/60 p-7 text-center shadow-2xl backdrop-blur-xl sm:p-9">
    <p className="text-sm font-black uppercase tracking-[.2em] text-rose-500">¡Partida en curso!</p>
    ...
    <button onClick={() => { sounds.click(); onVote(); }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-500 px-5 py-4 text-lg font-black text-white shadow-lg shadow-fuchsia-300/40 transition hover:-translate-y-0.5"><Vote size={20}/>Votar ahora</button>
  </motion.div></main>;
}
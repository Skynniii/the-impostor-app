  ...
  return <main className="flex min-h-screen items-center justify-center px-5 py-10">
    <AnimatePresence mode="wait">
      {!show ? <motion.div key={count} initial={{ scale: 0.4, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 1.6, opacity: 0 }} className="text-center">
        ...
        <div className="my-6 space-y-2">{allImpostors.map(i => <div key={i} className="flex items-center justify-center gap-2 rounded-xl bg-white/80 px-4 py-2.5 font-bold text-slate-700"><Skull size={16} className="text-rose-500"/>{names[i]}</div>)}</div>
